CREATE TABLE revival_works (
  ...
  updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, content_notice TEXT, popularity INTEGER, downloads INTEGER, group_id TEXT, group_primary INTEGER NOT NULL DEFAULT 1,
  UNIQUE (source, source_id)
);

CREATE INDEX idx_revival_works_downloads
  ON revival_works (status, downloads DESC);

CREATE INDEX idx_revival_works_group
  ON revival_works (group_id, group_primary DESC);

CREATE INDEX idx_revival_works_popularity
  ON revival_works (status, popularity DESC);

CREATE INDEX idx_revival_works_primary
...
CREATE INDEX revival_works_notice_idx
...
CREATE INDEX revival_works_source_updated_idx
...
CREATE INDEX revival_progress_viewer_idx
